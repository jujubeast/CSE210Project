...
class SimpleSearchOperationTest < ActionView::TestCase
  test "simple search operation 1" do
    
    puts "Test Simple search...\n"
    # create 5 stores
    ...
    search_op = SimpleSearchOperation.new("mexican")
    search_op.do_search
    view_data = search_op.view_data
    assert view_data.size == 2
    assert view_data[0].id == teststore3.id
    assert view_data[0].name == teststore3.name
    assert view_data[1].id == teststore4.id
    assert view_data[1].name == teststore4.name
  end
